    ...
    Vue.component('scroll-picker', {
        template: '\
			<div class="divide-bar">\
				...
				<div class="divide-bar-stage" id="divide-bar-stage" :style="{ height: stageHeigth + \'rem\' }" @click="handleEvent" @touchstart="handleEvent" @touchmove="handleEvent" @touchend="handleEvent">\
        ...
        data: function () {
            return {
                stageHeigth: 500/50,//滚动高度
                itemHeight: 60,//每个div高度
                rotateRate: 0,//每次旋转角度
                itemNum: 18,//单圈最大容纳数目
                zDistance: 0,//轴距
                realRotate: 0,//实际旋转度数
            ...
            maps: function (val) {
                ...
                if(this.map){
                    ...
                    this.maps.forEach(function (item, index) {
                        if (data&&data.defData === item.defData) {
                            thiz.select(index);
                        }
                    });
                }
